...
import 'package:kumari_drivers/Dialog/payment_dialog.dart';
...
class NewTripPage extends StatefulWidget {
  ...
  NewTripPage({super.key, this.newTripDetailsInfo, this.tripDetailsInfo});
...
class _NewTripPageState extends State<NewTripPage> {
  ...
  String buttonTitleText = "ARRIVED".tr();
  ...
  obtainDirectionAndDrawRoute(
    ...
    print("====================================$tripDetailsInfo.encodedPoints");
  ...
  endTripNow() async {
    ...
    String fareAmount = cMethods
        .calculateFareAmountFor4Seats(directionDetailsEndTripInfo!)
        .toString();
  ...
  Future<void> saveFareAmountToDriverTotalEarnings(String fareAmount) async {
    ...
    final snap = await driverEarningsRef.once();

    double previousTotalEarnings = 0.0; // Default value for previous earnings
    if (snap.snapshot.value != null) {
      try {
        previousTotalEarnings = double.parse(snap.snapshot.value.toString());
      } catch (e) {
        print('Error parsing previous total earnings: $e');
      }
    }

    double fareAmountForTrip = 0.0; // Default value for fare amount
    try {
    ...
    try {
      await driverEarningsRef.set(newTotalEarnings);
    } catch (e) {
      print('Error setting new total earnings: $e');
    }
  }
  ...
  saveDriverDataToTripInfo() async {
    Map<String, dynamic> driverDataMap = {
      ...
      "carDetails": carSeats + " - " + carModel + " - " + carNumber,
    };
